...
pub fn main() !void {
    ...
    for (std.mem.span(namespaces)) |namespaceZ| {
        ...
        file.close();
        var longer_file_name = try allocator.alloc(u8, output_path.len + file_name.len);
        defer allocator.free(longer_file_name);
        std.mem.copy(u8, longer_file_name[0..], output_path);
        std.mem.copy(u8, longer_file_name[output_path.len..], file_name);
