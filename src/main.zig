...
pub fn main() !void {
    ...
    // Load GIR
    const repository = gi.Repository.getDefault();
    for (includedir) |i| {
        const p = try allocator.dupeZ(u8, i);
        defer allocator.free(p);
        gi.Repository.prependSearchPath(p);
    }
    {
...
pub fn generateBindings(allocator: std.mem.Allocator, repository: *gi.Repository, output_dir: std.fs.Dir, pkg_config: struct {
    ...
    for (std.mem.span(repository.getLoadedNamespaces())) |namespaceZ| {
        ...
        defer allocator.free(filename);
        {
            ...
            for (std.mem.span(repository.getDependencies(namespace))) |dependencyZ| {
    ...
    if (data.get(namespace)) |prefix| {
        ...
        switch (info) {
            ...
            .property => |p| {
                ...
                try writer.print("({c}{c}) {}\n", .{ @as(u8, if (flags.readable) 'r' else '-'), @as(u8, if (flags.writable and !flags.construct_only) 'w' else '-'), p.getType() });
            },
            .signal => {
                const container = info.getContainer().?;
                const container_name = std.mem.span(container.getName().?);
