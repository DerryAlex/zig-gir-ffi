...
pub fn load(self: *Repository, io: Io, namespace: []const u8, version: ?[]const u8) Repository.Error!void {
    ...
    for (search_paths.items) |search_path| {
        var dir = cwd.openDir(io, search_path, .{ .iterate = true }) catch continue;
        defer dir.close(io);
        ...
        const file = dir.openFile(io, filename, .{}) catch continue;
        ...
        for (loaded_namespace.dependencies.items) |dep| try self.load(io, dep, null);
        // transive dependency
