...
static inline int maybe_allocate_on_stack(GITypeInfo *type_info)
{
    ...
    if (type == GI_TYPE_TAG_INTERFACE)
    {
        ...
        {
            if (g_struct_info_get_n_fields(interface) == 0) return 0;
            if (strcmp(g_base_info_get_name(interface), "Value") == 0 && strcmp(g_base_info_get_namespace(interface), "GObject") == 0) return 0;
            return 1;
        }
        if (info_type == GI_INFO_TYPE_UNION || info_type == GI_INFO_TYPE_BOXED || info_type == GI_INFO_TYPE_ENUM || info_type == GI_INFO_TYPE_FLAGS) return 1;
    }
...
void emit_c_function(GIBaseInfo *info, const char *name, const char *container_name, int is_deprecated, int is_container_struct)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        emit_type(type_info, g_arg_info_is_optional(arg_info) || g_arg_info_may_be_null(arg_info), 0, is_out_dir, 1, is_out_dir, direction == GI_DIRECTION_OUT && !g_arg_info_is_caller_allocates(arg_info));
    ...
    else emit_type(return_type_info, return_nullable || patch_return_nullable(return_type_info), 0, 0, 1, 1, 0);
...
void emit_callback(GIBaseInfo *info, const char *name, int is_deprecated, int type_only)
{
    ...
    if (!type_only)
    {
        ...
        if (config_enable_auto_function_comment)
        {
            printf("/// %s\n", name);
    ...
    if (g_callable_info_can_throw_gerror(info)) printf(", *?*core.Error");
    printf(") callconv(.C) ");
    int return_nullable = g_callable_info_may_return_null(info);
    GITypeInfo *return_type_info = g_callable_info_get_return_type(info);
    emit_type(return_type_info, return_nullable, 0, 0, 1, 1, 0);
    g_base_info_unref(return_type_info);
    if (!type_only) printf(";\n");
}
...
void emit_struct(GIBaseInfo *info, const char *name, int is_deprecated)
{
    ...
    else
    {
        ...
        int zero_filled = 0;
        if (strcmp(name, "Value") == 0 && strcmp(g_base_info_get_namespace(info), "GObject") == 0) zero_filled = 1;
        for (int i = 0; i < n; i++)
        {
            GIFieldInfo *field_info = g_struct_info_get_field(info, i);
            emit_field(field_info, i == 0, zero_filled);
        }
    }
...
void emit_type(GIBaseInfo *type_info, int optional, int is_slice, int is_out, int prefer_c, int prefer_mut, int callee_alloc_out)
{
        ...
        case GI_TYPE_TAG_ARRAY:
            ...
            switch (array_type)
            {
                ...
                default:
                    ...
                    emit_type(array_info, zero_terminated && (!is_basic_type(elem_type) || g_type_info_is_pointer(array_info)), 0, 0, 0, prefer_mut, 0);
        ...
        case GI_TYPE_TAG_INTERFACE:
            ...
            switch (info_type)
            {
                ...
                case GI_INFO_TYPE_FLAGS:
                    ...
                    if (callee_alloc_out)
                    {
                        if (info_type == GI_INFO_TYPE_STRUCT && g_struct_info_get_n_fields(interface) == 0) printf("*");
                    }
                    printf("%s.%s", namespace, name);
                    break;
                default:
...
{
    if (!config_enable_auto_function_comment) return;
    int is_method = g_callable_info_is_method(info);
    if (is_method)
    {
        printf("/// @self: ");
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        GITypeInfo *type_info = g_arg_info_get_type(arg_info);
        emit_type(type_info, arg_optional, 0, 0, 0, 0, 0);
        printf("\n");
        g_base_info_unref(type_info);
        g_base_info_unref(arg_info);
    }
    ...
    emit_type(return_type_info, return_nullable, 0, 0, 0, 1, 0);
    g_base_info_unref(return_type_info);
    printf("\n");
}
...
void emit_function_wrapper(GIBaseInfo *info, const char *name, const char *container_name, int is_deprecated, int is_container_struct, GIStructInfo *virt_class)
{
    ...
    int param_gvalue[n];
    for (int i = 0; i < n; i++) param_gvalue[i] = 0;
    for (int i = 0; i < n; i++)
    {
        GIArgInfo *arg_info = g_callable_info_get_arg(info, i);
        GIDirection direction = g_arg_info_get_direction(arg_info);
        ...
        {
            GIBaseInfo *interface = g_type_info_get_interface(type_info);
            if (strcmp(g_base_info_get_name(interface), "Value") == 0 && strcmp(g_base_info_get_namespace(interface), "GObject") == 0) param_gvalue[i] = 1;
            g_base_info_unref(interface);
        }
    ...
    for (int i = 0; i < n; i++)
    {
        if ((param_dir[i] & PARAM_IN) || param_caller_allocate[i])
        {
            ...
            GITypeInfo *type_info = g_arg_info_get_type(arg_info);
            emit_type(type_info, param_nullable[i] || param_optional[i], param_is_slice_ptr[i], param_dir[i] == PARAM_OUT && param_gvalue[i], 0, param_dir[i] & PARAM_OUT, 0);
            g_base_info_unref(type_info);
            g_base_info_unref(arg_info);
        }
    }
    ...
    else
    {
        ...
        if (!skip_return && !boolean_error)
        {
            ...
            if (is_gtk_widget(return_type_info) && strncmp(name, "new", 3) == 0) printf("%s", container_name);
            else emit_type(return_type_info, return_nullable || patch_return_nullable(return_type_info), 0, 0, 0, 1, 0);
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            emit_type(type_info, param_nullable[i] || param_optional[i], param_is_slice_ptr[i], 0, 0, param_dir[i] & PARAM_OUT, 0);
            if (multiple_return > 1) printf(",\n");
            g_base_info_unref(type_info);
            g_base_info_unref(arg_info);
        }
    ...
    for (int i = 0; i < n; i++)
    {
        if (param_slice_len_ptr_pos[i] != -1)
        {
            ...
            if ((param_dir[ptr_pos] & PARAM_IN) || param_caller_allocate[ptr_pos])
            {
                ...
                GITypeInfo *type_info = g_arg_info_get_type(arg_info);
                emit_type(type_info, 0, 0, 0, 0, 1, 0);
                printf(" = @intCast(");
                emit_type(type_info, 0, 0, 0, 0, 1, 0);
                printf(", ");
    ...
    else
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (param_dir[i] == PARAM_OUT && param_gvalue[i]) printf(".*");
            g_base_info_unref(type_info);
            g_base_info_unref(arg_info);
        }
        if (multiple_return > 1) printf(" }");
...
void emit_field(GIFieldInfo *field_info, int first_field, int is_zero_filled)
{
    ...
    emit_type(field_type_info, g_type_info_is_pointer(field_type_info) || is_callback(field_type_info), 0, 0, 0, 0, 0);
    if (first_field && is_instance(field_type_info) && !g_type_info_is_pointer(field_type_info)) printf(".cType()");
    if (is_zero_filled) {
        printf(" = std.mem.zeroes(");
        emit_type(field_type_info, g_type_info_is_pointer(field_type_info) || is_callback(field_type_info), 0, 0, 0, 0, 0);
        printf(")");
    }
    printf(",\n");
...
void emit_property(GIPropertyInfo *info, const char *container_name)
{
    ...
    if (getter != NULL)
    {
        ...
        printf("(%s) ", container_name);
        emit_type(type_info, 0, 0, 0, 1, 0, 0);
        printf(";\n");
    ...
    else if (flags & G_PARAM_READABLE)
    {
        ...
        emit_type(type_info, 0, 0, 0, 1, 0, 0);
        printf(" {\n");
        printf("        var property_value = core.GValue(");
        emit_type(type_info, 0, 0, 0, 1, 0, 0);
        printf(").new();\n");
        printf("        defer property_value.deinit();\n");
        printf("        self.callMethod(\"getProperty\", .{ \"%s\", property_value.toValue() });\n", property_name);
        printf("        return property_value.get();\n");
        printf("    }\n");
    }
    ...
    else if ((flags & G_PARAM_WRITABLE) && !(flags & G_PARAM_CONSTRUCT_ONLY))
    {
        ...
        printf(").new();\n");
        printf("        defer property_value.deinit();\n");
        printf("        property_value.set(value);\n");
        printf("        self.callMethod(\"setProperty\", .{ \"%s\", property_value.toValue() });\n", property_name);
        printf("    }\n");
    }
    printf("};\n");
    printf("\n");
